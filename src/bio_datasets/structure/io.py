        ...
        for entity_chain_ids, entity_id in zip(chain_ids, entity_ids):
            ...
            if not poly_seq_entity_mask.any():
                ...
                for chain_id in entity_chain_ids.split(","):
                    ...
                    missing_res_mask = ~np.isin(complete_res_id, chain_atoms.res_id)
                    ...
                    missing_atoms.set_annotation(
                        "entity_id", np.full(len(missing_atoms), entity_id).astype(int)
                    )
                    missing_atoms.set_annotation(
                        "auth_chain_id",
                    ...
                    complete_atoms = (
                        chain_atoms + missing_atoms
                    )  # extra annots get dropped
                    annots_to_concat = [
                        "altloc_id",
                        "auth_chain_id",
                        "auth_res_id",
                    ...
                    for annot in annots_to_concat:
                        complete_atoms.set_annotation(
                            annot,
                            np.concatenate(
                                [chain_atoms._annot[annot], missing_atoms._annot[annot]]
                            ),
                        )
                    ...
                    post_perm_res_changes = (
                        permuted_residue_starts_atom[1:]
                        != permuted_residue_starts_atom[:-1]
                    )
                    ...
                    _post_perm_res_index = (
                        np.cumsum(
                            get_residue_starts_mask(
                                complete_atoms, post_perm_residue_starts
                            )
                        )
                        - 1
                    )
                    ...
                    permuted_relative_atom_index = (
                        ...
                        - post_perm_residue_starts[_post_perm_res_index]
                    )

                    atom_perm = (
                        permuted_residue_starts_atom + permuted_relative_atom_index
                    )
